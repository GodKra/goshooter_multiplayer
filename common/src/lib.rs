...
#[derive(Clone, Debug)]
pub enum Packet {
    GameStart { width: u8, height: u8, pids: Vec<u8> }, // sent when starting game | sym: @

    PlayerCreate(u8), // new player | sym: +
    PlayerDestroy(u8), // player leaves | sym: -

    PlayerEvent { pid: u8, event: Event }, // { player id, event } | sym: $
    BulletPos { id: String, x: u8, y: u8 }, // { bullet id, x, y } | sym: *
    BulletDestroy(String), // sym: ^
...
impl Packet {
    ...
    pub fn read_from(stream: &mut impl Read) -> Result<Option<Self>, io::Error> {
        ...
        match first_byte {
            ...
            b'$' => {
                let pid = Self::read_byte(stream)?;
                let event = Event::get(Self::read_byte(stream)?).unwrap(); // todo handle invalid event
            ...
            _ => Err(io::Error::new(io::ErrorKind::InvalidData, "invalid first byte")), // handle
        }
    }
